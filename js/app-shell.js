(() => {
  ...
  Object.assign(window.App, {
    ...
    cacheEls(){
      const ids=['appShell','desktopNav','mobileNav','viewContainer','pageTitle','topbarEyebrow','globalSearch','quickAddButton','notificationButton','notificationBadge','profileButton','avatarInitials','saveState','toastRegion','confirmDialog','confirmTitle','confirmText','confirmActionButton','recordDialog','recordDialogBody','restoreInput','systemBannerTitle','systemBannerText'];
      ids.forEach(id=>this.els[id]=document.getElementById(id));
    },
    bindGlobalEvents(){
      this.els.quickAddButton.addEventListener('click',()=>this.navigate('new'));
      this.els.notificationButton?.addEventListener('click',()=>this.navigate('notifications'));
    ...
    async loadSettings(){
      ...
      merged.attentionFeedback={...base.attentionFeedback,...(value.attentionFeedback||{})};
    ...
    async loadData(){
      const [records,activities,notifications]=await Promise.all([AppDB.getAll('records'),AppDB.getAll('activities'),AppDB.getAll('notifications')]);
      this.state.records=records.sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt));
      this.state.activities=activities.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
      this.state.notifications=notifications.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
    ...
    enterApp(){this.els.appShell.classList.remove('is-hidden');this.updateAvatar();this.setConnectionBanner(navigator.onLine);this.updateNotificationBadge?.();this.routeFromHash(true);},
    ...
    async renderRoute(route){
      ...
      const titles={dashboard:'Resumo',notifications:'Notificações',new:'Novo Registo',records:'Registos',clients:'Clientes',statistics:'Estatísticas',designer:'Designer de Formulário',routing:'Encaminhamento',activity:'Atividade',productivity:'Produtividade',drafts:'Rascunhos',archive:'Arquivo',settings:'Configurações',help:'Guia de Utilização',profile:'Identificação Local',more:'Mais',edit:'Editar Registo'};
