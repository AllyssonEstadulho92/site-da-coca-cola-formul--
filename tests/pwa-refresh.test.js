...
assert.match(sw, /registo-avarias-v4\.5\.0/, 'O cache deve estar na V4.5.0.');
...
assert.match(sw, /\.\/js\/app-equipment-default-images\.js/, 'O módulo de ilustrações padrão deve estar no cache local.');
assert.match(sw, /\.\/js\/app-equipment-upload-hotfix\.js/, 'O hotfix de upload direto deve estar no cache local.');
assert.match(sw, /\.\/js\/equipment-directory-v43\.js/, 'O diretório específico V4.5 deve estar no cache local.');
...
assert.match(sw, /\.\/css\/equipment-models-v43\.css/, 'Os estilos do catálogo V4.5 devem estar no cache local.');
assert.match(refresh, /updateViaCache:\s*'none'/, 'O registo do Service Worker deve forçar verificação sem cache.');
assert.match(refresh, /registoAvariasSwReloadedV45/, 'A recarga automática deve estar alinhada com V4.5.');
