...
assert.match(sw, /registo-avarias-v4\.1\.0/, 'O cache deve estar na V4.1.0.');
...
assert.match(sw, /\.\/css\/equipment-images-v41\.css/, 'Os estilos V4.1 das imagens devem estar no cache local.');
assert.match(refresh, /updateViaCache:\s*'none'/, 'O registo do Service Worker deve forçar verificação sem cache.');
assert.match(refresh, /registoAvariasSwReloadedV41/, 'A recarga automática deve estar alinhada com V4.1.');
