...
assert.match(sw,/formularios-operacionais-v6\.2\.0-r1/);assert.match(sw,/async function networkFirst/);assert.match(sw,/event\.request\.destination==='script'/);assert.match(sw,/event\.request\.destination==='style'/);assert.match(sw,/cache:'no-store'/);
...
assert.equal(/js\/equipment\//.test(sw),false);assert.equal(/equipment-v5\.css/.test(sw),false);assert.match(refresh,/updateViaCache:'none'/);assert.match(refresh,/formulariosOperacionaisSwReloadedV620R1/);
console.log('PWA refresh tests V6.2.0-r1: OK');
