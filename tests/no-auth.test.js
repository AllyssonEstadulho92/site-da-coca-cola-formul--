...
assert.match(db,/DB_VERSION=6/);assert.match(db,/appVersion:'6\.3\.0'/);assert.match(db,/deleteObjectStore\('profiles'\)/);assert.equal(/getAll\('profiles'\)/.test(db),false);assert.equal(/objectStore\('profiles'\)\.put/.test(db),false);
assert.match(sw,/formularios-operacionais-v6\.3\.0/);assert.equal(/app-auth-(?:domain|adaptive)\.js/.test(sw),false);assert.equal(/auth-security\.css/.test(sw),false);
console.log('No-auth tests V6.3.0: OK');
