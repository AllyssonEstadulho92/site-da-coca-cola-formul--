...
assert.match(db,/DB_VERSION=5/);assert.match(db,/appVersion:'6\.1\.0'/);assert.match(db,/deleteObjectStore\('profiles'\)/);assert.equal(/getAll\('profiles'\)/.test(db),false);assert.equal(/objectStore\('profiles'\)\.put/.test(db),false);
assert.match(sw,/formularios-operacionais-v6\.1\.0/);assert.equal(/app-auth-(?:domain|adaptive)\.js/.test(sw),false);assert.equal(/auth-security\.css/.test(sw),false);
console.log('No-auth tests V6.1.0: OK');
