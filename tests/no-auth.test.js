...
assert.match(index, /Não utilize dados reais de clientes, informação SAP ou informação interna\/confidencial/i, 'O aviso público deve proibir dados reais e informação interna.');
...
assert.match(db, /DB_VERSION\s*=\s*4/, 'A base local deve permanecer na versão 4 para suportar imagens manuais.');
assert.match(db, /appVersion:\s*'5\.0\.1'/, 'Os backups devem identificar a aplicação V5.0.1.');
...
assert.match(sw, /registo-avarias-v5\.0\.1/, 'O cache PWA deve corresponder à V5.0.1.');
...
console.log('No-auth tests V5.0.1: OK');
